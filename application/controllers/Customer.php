<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Customer extends CI_Controller {


private $limit = 20;

	function __construct()

	{
		parent::__construct();
		$this->load->model('Customermodel','',TRUE);
		$this->load->model('Packagemodel','',TRUE);
		$this->load->helper(array('form', 'url'));
    	$this->load->library('form_validation');
	}

	public function index()
	{
		$session_data = $this->session->userdata('logged_in');
		$data['email'] = $session_data['email'];
		$data['session_data'] = $session_data;
		$uri_segment = 4;
		$offset = $this->uri->segment($uri_segment);
		
		// load data		
		$viewdata = $this->Customermodel->get_paged_list($this->limit, $offset)->result();
		
		$data["viewdata"] = $viewdata;
		$data['title'] = 'Customer';
		$data['action'] = "All Record";
				
		// generate pagination		
		$this->load->library('pagination');		
		$config['base_url'] = base_url()."customer/index/";
		$config['total_rows'] = $this->Customermodel->count_all();
		$config['per_page'] = $this->limit;
		$config['uri_segment'] = $uri_segment;
		$this->pagination->initialize($config);
		$data['pagination'] = $this->pagination->create_links();
		$data['message'] = $this->session->flashdata('message');		
		$data["num"] = $this->uri->segment(4);

		$this->load->view('header',$data);
		$this->load->view('customer/all',$data);
		$this->load->view('footer');
		
	}
	public function add()
	{
		
		$session_data = $this->session->userdata('logged_in');
		$data['email'] = $session_data['email'];
		$data['session_data'] = $session_data;
		$data['packagelist']=$this->Customermodel->get_paged_list();
		$data['title'] = 'Customer';
		$data['action'] = "Add Customer";
	
		$this->load->view('header',$data);
		$this->load->view('customer/add',$data);
		$this->load->view('footer');
	}
	public function addrecord()
	{
		$this->form_validation->set_rules('cno', 'Customer No', 'required');
        $this->form_validation->set_rules('cname', 'Customer Name', 'required');
        $this->form_validation->set_rules('address', 'Address', 'required');
        $this->form_validation->set_rules('city', 'City', 'required');
        $this->form_validation->set_rules('mobile1', 'Mobile 1', 'required');
     	$this->form_validation->set_rules('entrydate', 'Entry Date', 'required');


		
		if ($this->form_validation->run() == FALSE)
		{
			$this->add();
		}	
		else
		{	
			
			$data = array(   
						'cno' => $this->input->post('cno'),
						'cname' => $this->input->post('cname'),
						'address' => $this->input->post('address'),
						'city' => $this->input->post('city'),
						'mobile1' => $this->input->post('mobile1'),
						'mobiel2' => $this->input->post('mobile2'),
						'pid' => $this->input->post('pname'),
						'entrydate' => $this->input->post('entrydate')
					);   
			
		$id = $this->Customermodel->save($data);	
		$this->session->set_flashdata("message", "Record Added Successfully..."); 				
		redirect('welcome','refresh');	
		
		}
	}

	public function delete($id)
	{
		$this->Customermodel->delete($id);		
		$this->session->set_flashdata("message", "Record Deleted Successfully..."); 			
		redirect('package/','refresh');
	}
	
	public function edit($id)
	{			
		$data["editdata"] = $this->Customermodel->get_by_id($id)->row();	
		$data['packagelist']=$this->PackageModel->get_paged_list();
		$session_data = $this->session->userdata('logged_in');
		$data['email'] = $session_data['email'];
		$data['session_data'] = $session_data;				
		$data['title'] = 'Customer';
		$data['action'] = "Edit Record";
		
		
		$this->load->view('header',$data);
		$this->load->view('customer/edit',$data);
		$this->load->view('footer');
	}
	public function updaterecord()
	{
		
		$this->form_validation->set_rules('cno', 'Customer No', 'required');
        $this->form_validation->set_rules('cname', 'Customer Name', 'required');
        $this->form_validation->set_rules('address', 'Address', 'required');
        $this->form_validation->set_rules('city', 'City', 'required');
        $this->form_validation->set_rules('mobile1', 'Mobile 1', 'required');
      	$this->form_validation->set_rules('pname', 'Package Name', 'required');
		$this->form_validation->set_rules('entrydate', 'Entry Date', 'required');

                
		if ($this->form_validation->run() == FALSE)
		{
			$this->edit($this->input->post('cid'));
		}	
		else
		{	
				$data = array(                         
                    	'cno' => $this->input->post('cno'),
						'cname' => $this->input->post('cname'),
						'address' => $this->input->post('address'),
						'city' => $this->input->post('city'),
						'mobile1' => $this->input->post('mobile1'),
						'mobiel2' => $this->input->post('mobile2'),
						'pid' => $this->input->post('pname'),
						'entrydate' => $this->input->post('entrydate')
                );  	
			$id = $this->input->post('cid');		
			$this->Customermodel->update($id,$data);	
			$this->session->set_flashdata("message", "Record Updated Successfully..."); 									
			redirect('customer','refresh');	
		}
						
	}

}
